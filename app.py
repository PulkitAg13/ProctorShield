...
import random  # Import random for simulating noise detection
...
NOISE_SIMULATION_THRESHOLD = 20  # Set your noise threshold here
...
def detect_noise_level():
    # Function to simulate noise detection
    return random.randint(0, 100)  # Simulated noise levels from 0 to 100
...
def generate_frames():
    global exam_in_progress, face_detector, no_face_start_time, multiple_face_start_time, exam_window, current_window, noise_level, warning_issued, warning_time
        ...
        if not success:
            ...
            if noise_level > NOISE_SIMULATION_THRESHOLD:
                if not warning_issued:
                    cv2.putText(frame, 'Warning: Excessive noise detected!', (10, 150), 
                                cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                    warning_issued = True
                    warning_time = time.time()  # Start warning timer
                else:
                    # Check if 5 seconds have passed since the warning
                    if time.time() - warning_time > 5:
                        cv2.putText(frame, 'Terminating due to excessive noise.', (10, 180), 
                                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                        exam_termination_event.set()
            else:
                # Reset warning if noise level goes back to normal
                warning_issued = False

            cv2.putText(frame, f'Faces detected: {face_count}', (10, 30), 
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)

            # Add current window to the frame
            ...
            if exam_in_progress:
                if current_window != exam_window:
                    cv2.putText(frame, 'Tab switch detected. Terminating exam.', (10, 210), 
                                cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                    exam_termination_event.set()
